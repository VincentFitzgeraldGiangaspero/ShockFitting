...
//3D ADIMENSIONAL
  ...
  // normal velocity
  xd.at(2) = ((*ZRoeShd)(2,IV,ISH) * dx + (*ZRoeShd)(3,IV,ISH) * dy + (*ZRoeShd)(4,IV,ISH) * dz);
  xd.at(2) = xd.at(2)/(*ZRoeShd)(0,IV,ISH);
  ...
  u_tan[0]=((*ZRoeShd)(2,IV,ISH))/((*ZRoeShd)(0,IV,ISH))-xd.at(2)*dx;
  u_tan[1]=((*ZRoeShd)(3,IV,ISH))/((*ZRoeShd)(0,IV,ISH))-xd.at(2)*dy;
  u_tan[2]=((*ZRoeShd)(4,IV,ISH))/((*ZRoeShd)(0,IV,ISH))-xd.at(2)*dz;
  ...
  // pressure
  help = pow((*ZRoeShd)(2,IV,ISH),2) + pow((*ZRoeShd)(3,IV,ISH),2)+pow((*ZRoeShd)(4,IV,ISH),2);
  xd.at(1) = (PhysicsInfo::getGam()-1)/PhysicsInfo::getGam() *
 	  ((*ZRoeShd)(0,IV,ISH) * (*ZRoeShd)(1,IV,ISH) - 0.5 * help); // pressure
  R2 = sqrt(PhysicsInfo::getGam()*xd.at(1)/xd.at(0)) +
 		0.5 * (PhysicsInfo::getGam()-1) * xd.at(2);
...
// density
	xu.at(0) = ((*ZRoeShu)(0,IV,ISH)*((*ZRoeShu)(0,IV,ISH)));


  // normal velocity
	xu.at(2) = ((*ZRoeShu)(2,IV,ISH) * dx + (*ZRoeShu)(3,IV,ISH) * dy + (*ZRoeShu)(4,IV,ISH) * dz);
	xu.at(2) = xu.at(2)/(*ZRoeShu)(0,IV,ISH); // normal
  ...
  // tangential velocity
   ...
   // u_norm[2]=(*ZRoeShu)(3,IV,ISH)/ReferenceInfo::geturef()* dz;
	...
	u_tan[0]=((*ZRoeShu)(2,IV,ISH))/(*ZRoeShu)(0,IV,ISH)-xu.at(2)*dx;
	u_tan[1]=((*ZRoeShu)(3,IV,ISH))/(*ZRoeShu)(0,IV,ISH)-xu.at(2)*dy;
	u_tan[2]=((*ZRoeShu)(4,IV,ISH))/(*ZRoeShu)(0,IV,ISH)-xu.at(2)*dz;

	xu.at(3)=sqrt(u_tan[0]*u_tan[0]+u_tan[1]*u_tan[1]+u_tan[2]*u_tan[2]); // tan velocity

	dxt=u_tan[0]/xu.at(3);
	dyt=u_tan[1]/xu.at(3);
	dzt=u_tan[2]/xu.at(3);

	help = pow((*ZRoeShu)(2,IV,ISH),2) + pow((*ZRoeShu)(3,IV,ISH),2)+pow((*ZRoeShu)(4,IV,ISH),2);
	xu.at(1) = (PhysicsInfo::getGam()-1)/PhysicsInfo::getGam() *
...
//
    ...
    // u component
      z3=z1*(xd.at(2)*dx+xd.at(3)*dxt);

      // v component
      z4=z1*(xd.at(2)*dy+xd.at(3)*dyt);

      // w component
      z5=z1*(xd.at(2)*dz+xd.at(3)*dzt);
...
//*/
   ...
   z2 = z1*hh;
    ...
    //u component
     z3=z1*(xu.at(2)*dx+xu.at(3)*dxt);

     // v component
     z4=z1*(xu.at(2)*dy+xu.at(3)*dyt);

     // w component
     z5=z1*(xu.at(2)*dz+xu.at(3)*dzt);
