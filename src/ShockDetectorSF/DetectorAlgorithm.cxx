...
void DetectorAlgorithm::configure(OptionMap& cmap, const std::string& prefix)
{
  ...
  // configureDeps(cmap, m_useCGAL.ptr().get());
...
{
  LogToScreen(INFO,"DetectAlgorithm::using => "+ m_whichDetector.name()+"\n");
  ...
  //SurfaceReconstruction ShockPoints;
  ...
  //IdentifySpecialPoints mySpecialPoints;      bypassato per ora

  // create object compute normal vector to the shock points  da fare con cgal
  ...
  vector<double> m_XYZ(PhysicsInfo::getnbDim(),1);
  ...
  fprintf(gridfile,"%s %s %s","VARIABLES = \"x0\" \"x1\" \"x2\"",var.str().c_str(), "\n");
  ...
  for(unsigned IPOIN=0;IPOIN<npoin->at(0);IPOIN++) {
   for(unsigned IV=0;IV<PhysicsInfo::getnbDim();IV++) {
    fprintf(gridfile,"%20.14F %s",(*XYZ)(IV,IPOIN)," ");
   }
   for(unsigned IDOF=0;IDOF<(*ndof);IDOF++) {
  ...
  for(unsigned IPOIN=0; IPOIN<npoin->at(0); IPOIN++) {
   ...
   // the XYZ coordinates are actually not used
   m_param2prim.ptr()->transform(m_zroe,m_XYZ,m_prim);
 ...
 // remove when you work in param
 // prim.resize(npoin->at(0)*(*ndof));

//  // uso direttamente zroe -- prim(0)=P
//  for(unsigned IPOIN=0; IPOIN<npoin->at(0); IPOIN++) {
//    for(unsigned IV=0; IV<(*ndof); IV++) {
//      prim.at(IPOIN*(*ndof)+IV) = (*zroe)(IV,IPOIN);
//    }
//  }


  m_whichDetector.ptr()->detect(prim);    // GnoffoShockSensor

  ShockPointProcessingCGAL InitialShockPoints;


  InitialShockPoints.PointReconstruction();


  // ShockSurfaceMeshingCGAL ShockSurface;
  //
  // ShockSurface.SurfaceMeshing();

  LaplacianSmoothing3D SmoothSurface;

  SmoothSurface.Smooth();

  ShockSurfaceRemeshingMMG SurfaceRemeshing;

  SurfaceRemeshing.Remeshing();
  PhysicsInfo::setnbShPoint(nShockPoints->at(0));// check if it works

  cout << "-ShockPoints after MMG Remeshing " << nShockPoints->at(0) << "\n";
  ...
  ///2D
  // FILE* shockFitted;
  //   shockFitted = fopen("log/FittedShockCGAL.dat","w");
  //
  //   fprintf(shockFitted,"%s","TITLE = Detected shock polyline projected in XZ simmetric plane\n");
  //   fprintf(shockFitted,"%s","VARIABLES = \"x0\" \"x1\" \"x2\" \n");
  //   fprintf(shockFitted,"%s","ZONE T = \"Detected shockFitted\"\n");
  //   fprintf(shockFitted,"%s","STRANDID=0, SOLUTIONTIME=0 ");
  //   fprintf(shockFitted,"%s %u %s","I=",nShockPoints->at(ISH),", J=1, K=1, ZONETYPE=Ordered ");
  //   fprintf(shockFitted,"%s","DATAPACKING=POINT\n");
  //   fprintf(shockFitted,"%s","DT = (SINGLE, SINGLE, SINGLE)\n");
  //
  //   for(unsigned ISHPOIN=0;ISHPOIN<nShockPoints->at(ISH);ISHPOIN++) {
  //    fprintf(shockFitted,"%22.14F", XSh.at(ISHPOIN));
  //    fprintf(shockFitted,"%22.14F", YSh.at(ISHPOIN));
  //    fprintf(shockFitted,"%22.14F", ZSh.at(ISHPOIN));
  //    fprintf(shockFitted,"%s","\n");
  //   }
  //
  //   fclose(shockFitted);
  //
  ...
  // //  std::vector<double> ZSh(nShockPoints->at(ISH));      // aggiunta coordinata zeta
  // //
  // //
  // //  for(unsigned ISHPOIN=0;ISHPOIN<nShockPoints->at(ISH);ISHPOIN++) {
  // //   XSh.at(ISHPOIN) = (*XYZSh)(0,ISHPOIN,ISH);
  // //   YSh.at(ISHPOIN) = (*XYZSh)(1,ISHPOIN,ISH);
  // //   ZSh.at(ISHPOIN) = (*XYZSh)(2,ISHPOIN,ISH);
  // //  }
  //
  //   // LSM on XSh and YSh
  //
  //  if(m_fittingTechnique=="Ellipse")
  //   { LSM.fitEllipse(XSh,ZSh); }
  ...
  //  ZSh.resize(LSM.getNewPoints()); //YSh
  //  //  ZSh.
  //
  //  XSh = LSM.getXvector();
  //  ZSh = LSM.getYvector();
  ...
  // for(unsigned ISH=0;ISH<(*nShocks);ISH++) {
  //   for(unsigned ISHPOIN=0;ISHPOIN<nShockPoints->at(ISH);ISHPOIN++) {
  //    (*XYZSh)(0,ISHPOIN,ISH) = XSh.at(newIdISHPOIN.at(ISHPOIN));
  //    (*XYZSh)(1,ISHPOIN,ISH) = YSh.at(newIdISHPOIN.at(ISHPOIN));
  //    (*XYZSh)(2,ISHPOIN,ISH) = ZSh.at(newIdISHPOIN.at(ISHPOIN));
  ...
  //
  // // re-distribute the shock points    non serve, lo fa CGAL, é la 2 dichiarazione
  // newShPointsDistribution.distribute();
  //
  //
  // // plot shock polynline      OUTPUT DI CGAL
  // FILE* shockPolyLine;
  // shockPolyLine = fopen("log/ShockPolyline.dat","w");
  //
  // fprintf(shockPolyLine,"%s","TITLE = Shock polyline\n");
  // fprintf(shockPolyLine,"%s","VARIABLES = \"x0\" \"x1\" \"x2\" \n");
  ...
  //
  // for(unsigned ISHPOIN=0;ISHPOIN<nShockPoints->at(ISH);ISHPOIN++) {
  //  fprintf(shockPolyLine,"%22.14F",(*XYZSh)(0,ISHPOIN,ISH));
  //  fprintf(shockPolyLine,"%22.14F",(*XYZSh)(1,ISHPOIN,ISH));
  //  //   fprintf(shockPolyLine,"%22.14F",(*XYZSh)(2,ISHPOIN,ISH));    // aggiunta dimensione
  ...
  // create object compute normal vector to the shock points da fare con cgal
  FindShockSurfaceNormals3DCGAL shockSurfNormals;

  // create object evaluate upstream and downstream state for each shock point
  // ComputeShockPointsState3D shockPointsUpAndDownState;

  // create object assigning upstream and downstream state for each
  // shock point
  AssignShockPointState3D shockPointsUpAndDownState;

  // // compute the unit normal to the shock points     // Compute normals with CGAL
  shockSurfNormals.computeUnitNormals3D();
  //
  ...
  shockPointsUpAndDownState.extractDownstreamAndUpstreamPoints
   (shockSurfNormals.getUnitNormalsVector3D());
...
void DetectorAlgorithm::setAddress()            // qui ci sono solo punti, non edges
{
...
void DetectorAlgorithm::setPhysicsData()
{
  ...
  ShFaces = PhysicsData::getInstance().getData <Array3D <double> > ("ShFaces");
