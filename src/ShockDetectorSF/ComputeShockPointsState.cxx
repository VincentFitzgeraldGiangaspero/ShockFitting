...
extractDownstreamAndUpstreamPoints(Array3D<double> normals)
{
  ...
  m_shockLayerThickness *= MeshData::getInstance().getDXCELL();  // diventa un area? controlla
  ...
  for(unsigned ISH=0;ISH<(*nShocks);ISH++) {
   for(unsigned ISHPOIN=0;ISHPOIN<nShockPoints->at(ISH);ISHPOIN++) {
    ...
    for(unsigned IELEM=0;IELEM<nelem->at(0);IELEM++) {
     ...
     xp.at(3) = XYZShd(0,ISHPOIN,ISH);  // node to be located
     yp.at(3) = XYZShd(1,ISHPOIN,ISH);  // node to be located
    ...
    for(unsigned IELEM=0;IELEM<nelem->at(0);IELEM++) {
     ...
     xp.at(3) = XYZShu(0,ISHPOIN,ISH);  // node to be located
     yp.at(3) = XYZShu(1,ISHPOIN,ISH);  // node to be located
