...
void GnoffoShockSensor::detect(std::vector<double>& primitiveVar)
{
  ...
  for(unsigned IELEM=0;IELEM<nelem->at(0);IELEM++) {
    ...
    if(phi>m_maxPressRatio) {
      ...
      if(!alreadyCounted) {
       (*XYZSh)(0,ISHPOIN,ISH)=0.5*((*XYZ)(0,map_varMax(1,IELEM))+
                                  (*XYZ)(0,map_varMin(1,IELEM)));
       (*XYZSh)(1,ISHPOIN,ISH)=0.5*((*XYZ)(1,map_varMax(1,IELEM))+
                                  (*XYZ)(1,map_varMin(1,IELEM)));

       (*XYZSh)(2,ISHPOIN,ISH)=0.5*((*XYZ)(2,map_varMax(1,IELEM))+  // aggiunta la z ma non sono sicuro di questa media
                                  (*XYZ)(2,map_varMin(1,IELEM)));
  ...
  if(ISHPOIN<2) {
   ...
   cout << " number of extracted shock points is = " ,
   cout <<  ISHPOIN ;
   cout << " \n";
  ...
  fprintf(shockPolyLine,"%s","DT = (SINGLE, SINGLE, SINGLE)\n");
  ...
  for(unsigned ISHPOIN=0;ISHPOIN<nShockPoints->at(ISH);ISHPOIN++) {
   ...
   fprintf(shockPolyLine,"%22.14F",(*XYZSh)(2,ISHPOIN,ISH));    //aggiunta la z
...
void GnoffoShockSensor::setSize()
{
  ...
  nShockEdges->resize(PhysicsInfo::getnbShMax());   // Sono edge noi abbiamo face
