					 ...
					 vector <double>& m_zroe)
{
  ...
  if (PhysicsInfo::getnbDim()==3){ //3D
    ...
    rho = m_prim.at(0) / m_prim.at(4) / ReferenceInfo::getRgas()/ 
          ReferenceInfo::getTref() * ReferenceInfo::geturef() * ReferenceInfo::geturef();
    sqrtr = sqrt(rho);
    kinetic = pow(m_prim.at(1),2)+pow(m_prim.at(2),2)+pow(m_prim.at(3),2);
    kinetic = kinetic*0.5;
    help = ReferenceInfo::getgam()/(ReferenceInfo::getgam()-1);
    h = help * ReferenceInfo::getRgas() * m_prim.at(4) + kinetic;

    m_zroe.at(4) = sqrtr * m_prim.at(3);
    ...
    m_XY.at(2) = m_XY.at(2)/ ReferenceInfo::getLref();
  } 
